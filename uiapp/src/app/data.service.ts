...
export class DataService {
  ...
  <h3>Data base (SQL)</h3>
  Among the technologies I've worked on, I want to highlight the SQL. I have already worked with several databases, from simple CRUD requests to complex SQL queries. I remember that one time I had to perform data loss analysis due to a server crash.
  ...
  I cound say that, I have never been a frontend developer, I don't think I have much creativity for exquisite appearance details, but in the last company I worked for, I was in charge of developing a completely frontend application, with graphs and lists that would be analyzed and monitored for the department's managers. Basically, its features were showing data of sales, inventory, and other information for a large supermarket chain.<br>
  Back then, frontend frameworks like Angular weren't popular, so I had to resort to pure Javascript and Jquery, to achive this challange.<br>
  In the end, I managed to make an application better than I expected. I built a structure so that each logged-in user could choose data from related tables and than create they own dynamic chart.<br>
